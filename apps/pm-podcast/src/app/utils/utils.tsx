...
export const pmDsUtilsTimeFormatter = (milliseconds: number, format: TTimeFormats): number | string | Partial<TTimeObject> => {
...
export const pmDsUtilsDateFormatter = (date: string, format: TDateFormats ): string =>{
...
export const pmDsUtilsFieldFormatter = (value: string | number, format: TDateFormats | TTimeFormats)=> {
  if(!value){
    return 'N|D';
  }
  return format.includes('time:') ? pmDsUtilsTimeFormatter(value as number, format) : pmDsUtilsDateFormatter(value as string, format);
}
